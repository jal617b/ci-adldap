		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					if($is_image)
					{
						#$file_url_anchor .= ' class="image-thumbnail" target="_blank"><img src="'.$file_url.'" height="50px">';
						$file_url_anchor .= ' class="image-thumbnail" target="_blank">'.$this->character_limiter($value,$this->character_limiter,'...',true);
					}
					else
					{
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->unset_filter			= $this->unset_filter;
	...
	protected function showAddForm()
	{
		...
		$data->unset_save_button_only	= $this->unset_save_button_only;
		$data->note	= $this->note;
		$data->unique_hash			= $this->get_method_hash();
...
#jaleonardo 
	protected $unset_save_button_only	= false;
	protected $note	= false;
	protected $unset_filter = false;
	...
	public function unset_save_button_only(){
		$this->unset_save_button_only = true;
		return $this;
	}
	...
	public function unset_filter()
    {
        $this->unset_filter = true;
	 ...
	 * @param $url_callback
	 */
	public function add_action( $label, $image_url = '', $link_url = '', $css_class = '', $url_callback = null, $action_target = '_self',$show_as_modal = '' )
...
class UploadHandler
{
    ...
    private function trim_file_name($name, $type) {
	   ...
	   $file_name = date('YmdHis').'-'.$this->_transliterate_characters($file_name);
